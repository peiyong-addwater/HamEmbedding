...
def su4_single_conv_encoding(theta, data, wires):
    """
    ...
    for i in range(num_layers):
        SU4(padded_data[15*i:15*(i+1)], wires=wires)
        SU4(theta[15*i:15*(i+1)], wires=wires)


def conv_repuload_encoding(theta, data, wires):
    num_row, num_columns = data.shape[0], data.shape[1]
    for j in range(num_columns):
        for i in range(0, num_row+1, 2):
            row_data = data[i]
            su4_single_conv_encoding(theta,  row_data[j], wires=[wires[i], wires[i + 1]])
...
if __name__ == '__main__':
    ...
    def conv_net(theta,  conv_weights, last_layer_params, image_conv_extract):
        num_conv_layers = conv_weights.shape[1]
        wires = list(range(num_wires))
        for wire in wires:
            qml.Hadamard(wires=wire)
        qml.Barrier(wires=wires, only_visual=True)
        conv_repuload_encoding(theta, image_conv_extract, wires)
        qml.Barrier(wires=wires, only_visual=True)
    ...
    def init_weights():
        ...
        return jnp.array(theta), jnp.array(conv_weights), jnp.array(weights_last)
    ...
    @jax.jit
    def compute_out(theta, conv_weights, weights_last, features, labels):
        out = lambda theta,conv_weights, weights_last, feature, label:conv_net(theta, conv_weights, weights_last, feature)
        return jax.vmap(out, in_axes=(None, None, None, 0, 0), out_axes=0)(
            theta,  conv_weights, weights_last, features, labels
        )

    def compute_accuracy(theta, conv_weights, weights_last, features, labels):
        out = compute_out(theta, conv_weights,weights_last, features, labels)
        pred = jnp.argmax(out, axis=1)
        return jnp.sum(jnp.array(pred == labels).astype(int)) / len(out)

    def compute_cost(theta, conv_weights, weights_last, features, labels):
        logits = compute_out(theta, conv_weights, weights_last, features, labels)
    ...
    def train_qcnn(n_train, n_test, n_epochs, rep):
        ...
        theta, conv_weights, weights_last = init_weights()
        ...
        opt_state = optimizer.init((theta, conv_weights, weights_last))
        ...
        for step in range(n_epochs):
            ...
            train_cost, grad_circuit = value_and_grad(theta, conv_weights, weights_last, x_train, y_train)
            updates, opt_state = optimizer.update(grad_circuit, opt_state)
            #print(updates)
            theta, conv_weights, weights_last = optax.apply_updates((theta, conv_weights, weights_last), updates)
            train_cost_epochs.append(train_cost)
            # compute accuracy on training data
            train_acc = compute_accuracy(theta, conv_weights, weights_last, x_train,
                                         y_train)
            train_acc_epochs.append(train_acc)
            # compute accuracy and cost on testing data
            test_out = compute_out(theta, conv_weights, weights_last, x_test,
                                   y_test)
    ...
    plt.savefig(f"fashion-mnist-4class-su4-encoding-no-compact-kernel-{KERNEL_SIZE[0]}-stride{STRIDE[0]}-results-{n_test}-test-{n_reps}-reps.pdf")
