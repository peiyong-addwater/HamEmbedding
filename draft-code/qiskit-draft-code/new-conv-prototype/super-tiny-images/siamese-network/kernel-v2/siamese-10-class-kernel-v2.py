...
def backbone_net(data_for_entire_2x2_feature_map, params):
    """
    single kernel requires 105+18 parameters
    three consective linear transformation requires 15*3=45 parameters
    two consective non-linear transformation requires 15*2*2=60 parameters
    total number of parameters: 105+18+45+60=228
    ...
    conv_kernel_param = params[:105]
    pooling_param = params[105:105+18]
    linear_params = params[105+18:105+18+45]
    non_linear_transformation_param = params[105+18+45:105+18+45+60]
    qubit_counter = 0
    for i in range(2):
        for j in range(2):
    ...
    circ.compose(su4_circuit(linear_params[:15]), qubits=[qreg[0], qreg[1]], inplace=True)
    circ.compose(su4_circuit(linear_params[15:30]), qubits=[qreg[1], qreg[2]], inplace=True)
    circ.compose(su4_circuit(linear_params[30:45]), qubits=[qreg[2], qreg[3]], inplace=True)
    circ.barrier()
    # non-linear transformation, just like those in the kernel
    circ.measure(qreg[0], creg[3])
    circ.measure(qreg[2], creg[4])
    circ.barrier()
    circ.compose(su4_circuit(non_linear_transformation_param[:15]), qubits=[qreg[0], qreg[1]], inplace=True)
    circ.compose(su4_circuit(non_linear_transformation_param[15:30]), qubits=[qreg[2], qreg[3]], inplace=True)
    circ.measure(qreg[1], creg[3])
    circ.measure(qreg[3], creg[4])
    circ.barrier()
    circ.compose(su4_circuit(non_linear_transformation_param[30:45]), qubits=[qreg[0], qreg[1]], inplace=True)
    circ.compose(su4_circuit(non_linear_transformation_param[45:60]), qubits=[qreg[2], qreg[3]], inplace=True)
...
parameter_backbone = ParameterVector("θ", length=228)
backbone = backbone_net(data, parameter_backbone)
backbone.draw(output='mpl', filename='backbone-5x5-input-8x8.png', style='bw')
...
siamese_circ.draw(output='mpl', filename='siamese-v2-conv-5x5.png', style='bw', scale=0.5)
