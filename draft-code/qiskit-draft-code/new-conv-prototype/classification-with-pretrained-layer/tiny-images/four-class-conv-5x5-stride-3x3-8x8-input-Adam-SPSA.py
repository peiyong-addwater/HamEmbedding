...
if __name__ == '__main__':
    ...
    pretrained_weights_filename = "/home/peiyongw/Desktop/Research/QML-ImageClassification/draft-code/qiskit-draft-code/new-conv-prototype/super-tiny-images/siamese-network/20230403-050802_siamese-10-class-qiskit-mnist-5x5-conv-multiclass-tiny-image-results-3-img_per_class-COBYLA.json"
    with open(pretrained_weights_filename, 'r') as f:
        pretrain_res = json.load(f)
        pretrained_weights = pretrain_res['params']


    def pretrained_conv_layer_1(data_for_entire_2x2_feature_map):
        """
        8 qubits
        ...
        conv_kernel_param = pretrained_weights[:45]
        pooling_param = pretrained_weights[45:]
        qubit_counter = 0
        for i in range(2):
            for j in range(2):
    ...
    def full_circ(prepared_data, params):
        """
        conv-1 requires 45 + 18 pretrained parameters
        ...
        final_layer_parameters = params
        circ = QuantumCircuit(qreg, pooling_measure, classification_reg)
        # conv-1
        circ.compose(pretrained_conv_layer_1(prepared_data), qubits=qreg, clbits=pooling_measure, inplace=True)
        circ.barrier(qreg)
        # final layer
    ...
    plt.savefig(f"{nowtime()}-qiskit-fashion-mnist-5x5-conv-4-class-tiny-image-pretrained-weights-{n_test}-test-{n_reps}-reps-Adam-SPSA.pdf")
