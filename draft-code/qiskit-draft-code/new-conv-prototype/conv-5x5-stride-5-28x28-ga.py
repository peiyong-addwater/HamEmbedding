...
# rng = np.random.default_rng(seed=42)
# data = load_data(10,10,rng)[0][0]
# params = ParameterVector('w', length=15*4+45+18+ 15+3*8+15+3*4*2 + 15)c
...
if __name__ == '__main__':
    ...
    def train_model(n_train, n_test, rep, rng):
        ...
        params = np.random.random((init_pop, 15*4+45+18+ 15+3*8+15+3*4*2 + 15))
        ...
        def on_generation(ga_instance):
            ...
            print(
                f"-------Rep={rep},n_train={n_train},n_test={n_test},generation={generation},train acc="
                f"{round(avg_best_solutions_acc, 5)},test acc={round(test_acc, 5)},avg gen time="
                f"{round(avg_gen_time, 5)},time till now={round(ttn, 2)}")
        ...
        ga_instance = pygad.GA(
            ...
            gene_space=[{'low': 0.0, 'high': 2*np.pi}] * (15*4+45+18+ 15+3*8+15+3*4*2 + 15),
    ...
    plt.savefig(f"qiskit-fashion-mnist-5x5-conv-ga-train-results-{n_test}-test-{n_reps}-reps.pdf")
