...
if __name__ == '__main__':
    ...
    for i in range(n_reps):
        res[i] = dict()
        res[i]["optmization_process"], res[i]["optmized_params"] = train_model(n_train=n_train, n_test=n_test, n_epochs=n_epochs, rep=i, rng=rng, params=params)

    with open(f"noisy-qiskit-fashion-mnist-5x5-conv-multiclass-tiny-image-results-{n_train}-train-{n_test}-test-{n_reps}-reps.json", 'w') as f:
